...
public class DataGenerator {
  ...
  /**
   * Generates random username. To achieve some proper randomization - preferable minimum length is
   * 10
   *
  ...
  public static String getRandomPassword(int length, boolean includeSpecials) {
    StringBuilder result = new StringBuilder();
    if (includeSpecials) {
      char[] charsPlusSpecials = ArrayUtils
          .addAll(CHARS.toCharArray(), SPECIAL_CHARS.toCharArray());
      for (int x = 0; x < length; x++) {
